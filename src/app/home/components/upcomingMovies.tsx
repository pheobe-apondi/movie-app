...
const UpcomingMovies: React.FC = () => {
  const { data: movies, loading, error } = useFetchUpcomingMovies();
  ...
  const isFavourite = (movie: Movie) => favourites.some((m) => m.id === movie.id);
  ...
  if (error) return <p>Error loading upcoming movies: {error}</p>;
  if (!movies) return <p>No upcoming movies found.</p>;

  return (
    <>
      <h1 className="text-3xl p-3 font-black">Upcoming Movies</h1>
      <div className="flex overflow-x-auto space-x-4 p-4 scrollbar-hide hide-scrollbar">
        {movies.map((movie) =>
          movie.poster_path ? (
            <div key={movie.id} className="relative min-w-[150px] flex-shrink-0">
              <div
